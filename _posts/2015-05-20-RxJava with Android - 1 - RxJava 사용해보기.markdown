...
바로 누군가 구독(subscribe)하고 있지 않는다고 하면 이벤트를 발생시키지 않습니다.
그리고 이부분은 Observable - Subject 의 중요한 차이점이 됩니다.
이 부분은 지금은 그냥 읽고 넘어가셔 되는 부분입니다. 그리고 RxJava를 계속 사용하다보면 감이 잡힐겁니다.
...
다른 RxJava 관련 자료들을 보시다보면 용어들을 서로 다르게 사용되여 혼란스러우실수 있을 것 같아서, 미리 여기선 어떤 용어를 사용하는지 이야기를 드리도록 하겠습니다.
여기서 사용되는 용어가 적당한 용어가 아닐수도 있습니다. 어색하다던지 다른 방법으로 설명을 하였으면 더 좋았겠다라는 의견이 있으시면 자세히 설명해주시면 충분히 고민해보도록 하겠습니다.

- 이벤트(event): 흔히 아이템이라고도 설명하기도 합니다. 구독자들에게 전달되는 데이터를 의미하며 주로 클릭이벤트, 상태이벤트 등도 있지만 API 응답같은 경우도 이벤트가 될 수 있습니다.
- 구독(subscribe) : Subscriber 가 이벤트를 전달받기 위해 하는 행위를 이야기합니다. RxJava 에서 subscribe() 메소드를 통해 이뤄집니다.
- 관찰(observe) : 여기서 관찰은 위의 구독보다는 좀 더 넓은범위의 행위를 이야기합니다. (구독을하게 되면 관찰을 하는건 맞습니다만 관찰을 한다고 구독을 하는것은 아닙니다.) RxJava 를 사용하다보면 작은 Observable 컴포넌트들을 서로 연결(map() 등이 이에 해당합니다.)하여 프로그래밍을 하는데 , 이 과정에서 Observable 은 다른 Observable 을 관찰하게 됩니다. 물론 Subscriber 는 구독을 통해서 Observable 을 관찰을 하게 됩니다.
...
`subscribeOn()` 는 누군가 Observable 에 __구독(subscribe)이 이루어지는 thread 를 지정__합니다.
...
`observeOn()` Observable 이 __이벤트를 전파할때, 즉 관찰자에게 전달될때 사용되는 thread 를 지정__합니다.
...
위 코드에서는 observable > observable2 > subscriber 로 이벤트가 전파가 됩니다. 즉 observable 의 이벤트에 관심이 있는 녀석은 subscriber 가 아니라 observable2 가 되게 됩니다.
...
> 여기서 observer2 는 구독자(subscriber)는 아니구요. 교환자(operator) 인 map()에 의해 변형된 Observable 입니다. 
그리고 교환자는 구독의 행위가 아니기때문에 __observable2 만 가지고는 이벤트가 발생하지는 않습니다.__
...
그리고 [Grokking RxJava, Part 1: The Basics](http://blog.danlew.net/2014/09/15/grokking-rxjava-part-1) 를 꼭 읽어보시길 권장해드립니다. 바쁘시다면 포스팅 가장 하단에 __"So What? 의 Key Idea"__ 만이라도 꼭 읽어보시길 바랍니다. Observable 과 Subscriber 를 매우 잘 설명한 문구라고 생각합니다.
